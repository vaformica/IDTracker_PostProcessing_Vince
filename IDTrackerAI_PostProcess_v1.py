...
def write_roi_outputs(df: pd.DataFrame, rois, beetle_ids, fps: float, out_base: str):
    ...
    # Store per-frame speeds (step lengths) for each beetle in each ROI
    roi_speeds = {bid: {roi["name"]: [] for roi in rois} for bid in beetle_ids}
    ...
    for frame_idx, row in df.iterrows():
        ...
        for bid in beetle_ids:
            ...
            # distance inside rois & record per-frame speeds
            ...
            if valid and not (math.isnan(prev_x) or math.isnan(prev_y)):
                ...
                for roi in rois:
                    ...
                    if prev_inside[bid][roi_name]:
                        ...
                        roi_speeds[bid][roi_name].append(step)
            prev_pos[bid] = (x, y)
    ...
    # Helper: compute median speed over 10-frame windows
    def _median_speed_over_10_frames(speeds: list[float]) -> float:
        if not speeds:
            return 0.0
        n = len(speeds)
        # speeds are per-frame step lengths (px/frame)
        if n < 10:
            # not enough for a 10-frame window, fall back to median of what we have
            return float(np.median(speeds))
        window_avgs = []
        for i in range(n - 10 + 1):
            window = speeds[i:i+10]
            window_avgs.append(sum(window) / 10.0)
        return float(np.median(window_avgs))
...
Post-processing pipeline for IDTrackerAI trajectories with GUI.

Features:
---------
- User-friendly GUI with the following rows:
    0. Base directory scan (auto-fills CSV and TOML if possible)
    1. Trajectories CSV file
    2. ROI TOML file (optional)
    3. Distance threshold (px)
    4. Min contact duration (seconds)
    5. Output directory (defaults to Post_Processing_Output next to CSV or base dir)
    6. Run button

- Auto-detects trajectories.csv and .toml in the selected base directory.
- Outputs:
    1. InqScribe tab-delimited file of proximity/NaN events
    2. Multi-page PDF: one page per beetle, one combined, ROIs drawn under tracks
    3. Pairwise distances CSV
    4. ROI event CSV and ROI summary CSV (if TOML provided)
       - ROI summary includes: frames in ROI, % time, total distance, median speed over 10-frame windows

Detailed comments are provided throughout for clarity and maintainability.
"""
...
# =============================================================================
# 1. Format seconds as HH:MM:SS,mmm (for InqScribe)
# =============================================================================
def fmt_hhmmss_comma_ms(seconds: float) -> str:
    """
    Convert seconds (float) to 'HH:MM:SS,mmm' string for InqScribe.
    """
    if seconds < 0:
        seconds = 0.0
    h = int(seconds // 3600)
...
# =============================================================================
# 2. Segment mask frames: find runs of True >= min_len_frames
# =============================================================================
def segment_mask_frames(mask: np.ndarray, min_len_frames: int):
    """
    Given a boolean array (mask), return (start_frame, end_frame) tuples
    for all contiguous True runs of at least min_len_frames.
    """
    segments = []
    in_seg = False
    start = 0
...
# =============================================================================
# 3. Discover all tracked IDs (xN/yN pairs)
# =============================================================================
def discover_ids(df: pd.DataFrame):
    """
    Find all IDs present as xN/yN columns in the DataFrame.
...
# =============================================================================
# 4. Make multi-page PDF of tracks (ROIs underlaid)
# =============================================================================
def make_tracks_pdf(df: pd.DataFrame, pdf_path: str, rois=None):
    """
    Create a multi-page PDF:
    - One page per individual ID (xN/yN)
    - One final page with all IDs together
    - ROIs (if provided) are drawn in black underneath on every page
    - Tracks are colored consistently across pages
    """
    # Find all IDs present as xN/yN
    ids = []
    for col in df.columns:
        m = re.fullmatch(r"x(\d+)", col)
    ...
    # Compute global x/y limits for all tracks (for consistent axis scaling)
    all_x = np.concatenate(
        [df[f"x{i}"].to_numpy(dtype=float) for i in ids]
    )
    all_y = np.concatenate(
        [df[f"y{i}"].to_numpy(dtype=float) for i in ids]
    )
    all_x = all_x[~np.isnan(all_x)]
    all_y = all_y[~np.isnan(all_y)]
    if all_x.size > 0 and all_y.size > 0:
    ...
    # Color cycle for tracks (consistent)
    import itertools
    prop_cycle = plt.rcParams["axes.prop_cycle"].by_key().get("color", ["C0", "C1", "C2", "C3"])
    color_cycle = itertools.cycle(prop_cycle)
    id_to_color = {i: next(color_cycle) for i in ids}
...
# =============================================================================
# 5. Write pairwise distances for all ID pairs (frame-by-frame)
# =============================================================================
def write_pairwise_distances(df: pd.DataFrame, ids, base: str,
                             fps: float, threshold_px: float) -> str:
    """
    For every pair of IDs (i,j), write frame-by-frame distances and contact mask.
    """
    out_path = f"{base}_pairwise_distances_{int(threshold_px)}px_{fps:.3f}fps.csv"
    num_frames = len(df)
    frames = np.arange(num_frames, dtype=int)
...
# =============================================================================
# 6. ROI helpers: parse, load, point-in-poly, write ROI outputs
# =============================================================================
def parse_roi_string(s: str):
    """
    Parse a string representing an ROI polygon (as in TOML) into a list of (x, y) tuples.
    """
    m = re.search(r"\[\s*\[.*\]\s*\]", s)
    if not m:
        raise ValueError(f"Could not parse ROI polygon from: {s}")
...
def load_rois_from_toml(toml_path: str):
    """
    Load ROI polygons from a TOML file. Expects a key 'roi_list' containing strings.
    The first ROI is assumed to be "arena", subsequent ones "bracketN".
    """
    with open(toml_path, "rb") as f:
        data = tomllib.load(f)
    roi_list = data.get("roi_list")
...
def write_roi_outputs(df: pd.DataFrame, rois, beetle_ids, fps: float, out_base: str):
    """
    For each beetle and ROI, record:
      - Enter/exit events (in roi_events.csv)
      - Summary stats (in roi_summary.csv): frames in ROI, % time, total distance, median speed over 10-frame windows
    """
    num_frames = len(df)
    prev_inside = {bid: {roi["name"]: False for roi in rois} for bid in beetle_ids}
    frames_in_roi = {bid: {roi["name"]: 0 for roi in rois} for bid in beetle_ids}
...
def process_trajectories(csv_path: str,
    ...
    - Computes proximity/NaN events for InqScribe
    - Writes multi-page PDF, pairwise distances, ROI outputs (if any)
    Returns paths to output files and number of events.
    """
    df = pd.read_csv(csv_path)
    ids = discover_ids(df)
    if not ids:
...
# =============================================================================
# 8. GUI (Tkinter): base-directory scan, TOML auto-detect, output dir default, etc.
# =============================================================================
def run_gui():
    """
    Launches the Tkinter GUI for user configuration and running the pipeline.
    GUI rows (top to bottom):
        0. Base directory (scan for files)
        1. Trajectories CSV
        2. ROI TOML
        3. Distance threshold (px)
        4. Min contact duration (s)
        5. Output directory
        6. Run
    """
    root = tk.Tk()
        ...
        for rootdir, _dirs, files in os.walk(d):
            for fname in files:
                ...
                if lower == "trajectories.csv":
                    ...
                    toml_files.append(full)
        # Auto-fill CSV and TOML if only one found
        if len(traj_files) == 1:
            csv_var.set(traj_files[0])
        elif len(traj_files) > 1:
            messagebox.showwarning(
    ...
    # Layout (rows 0–6 as specified)
    frm = tk.Frame(root, padx=10, pady=10)
    frm.pack(fill="both", expand=True)
    # Row 0: Base directory
    tk.Label(frm, text="Base directory:").grid(row=0, column=0, sticky="w")
    tk.Entry(frm, textvariable=basedir_var, width=50).grid(row=0, column=1, sticky="we")
    tk.Button(frm, text="Browse…", command=choose_basedir).grid(row=0, column=2, padx=5)
    # Row 1: CSV
    ...
    # Row 3: Distance threshold (px)
    tk.Label(frm, text="Distance threshold (px):").grid(row=3, column=0, sticky="w")
    tk.Entry(frm, textvariable=thresh_var, width=10).grid(row=3, column=1, sticky="w")
    # Row 4: Min contact duration (s)
    tk.Label(frm, text="Min contact duration (s):").grid(row=4, column=0, sticky="w")
    dur_entry = tk.Entry(frm, textvariable=dur_var, width=10)
    dur_entry.grid(row=4, column=1, sticky="w")
    dur_entry.bind("<FocusOut>", update_frames_hint)
...
# =============================================================================
# 9. CLI fallback (if Tkinter is not available)
# =============================================================================
def main():
    """
    Entry point: runs GUI if possible, otherwise falls back to CLI.
    """
    if _HAS_TK:
        run_gui()
    else:
